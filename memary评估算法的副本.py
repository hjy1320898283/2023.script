...
def main (wideth,depth):
    output = []
    list_wideth = []
    print ('the size you choose is width : %d , depth is %d ' % (wideth,depth))   #1.tell you the size you apply
    ...
    for item in list_all:                                                        #3.Traversal database
        if item [1] == depth_i:                                                  #4.search the data in step2
            list_wideth.append(item[0])                                           #5.make the satisfied depth a new list
            output.append(item)                                                  #6.make a new list that satisfy width
    wideth_i = min(list_wideth, key=lambda x:abs(x-wideth))                       #7.select the most similar value to depth
